// V2 Experience — minimal click-to-expand rows, blue accent
    ...
    >
      ...
      >
        ...
        <span className={`w-2 h-2 rounded-full shrink-0
          ${item.type === 'current' ? 'bg-blue-400 animate-pulse' : 'bg-white/10'}`} />
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-blue-400 text-sm font-medium mt-0.5">{item.company}
            <span className="text-slate-500 font-normal"> · {item.location}</span>
          </p>
        </div>

        <span className="text-slate-500 text-sm shrink-0 hidden md:block">{item.period}</span>

        <span className="w-8 h-8 rounded-full border border-white/10 flex items-center justify-center
                         text-slate-500 group-hover:border-blue-400/40 group-hover:text-blue-400
                         transition-all duration-200 shrink-0">
          {open ? <HiMinus size={14} /> : <HiPlus size={14} />}
        </span>
      </button>

      <p className="text-slate-500 text-xs md:hidden -mt-3 mb-3 pl-8">{item.period}</p>
